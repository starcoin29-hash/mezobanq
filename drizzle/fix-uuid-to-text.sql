-- Migration: Convert user ID columns from uuid → text (for Clerk string IDs)
-- This must be done in order: drop FKs → alter types → re-add FKs
...
-- 2. Clear any existing data with old uuid format (won't match Clerk IDs anyway)
...
ALTER TABLE "gift_cards" ADD CONSTRAINT "gift_cards_creator_id_users_id_fk"
  FOREIGN KEY ("creator_id") REFERENCES "users"("id");
ALTER TABLE "gift_cards" ADD CONSTRAINT "gift_cards_claimer_id_users_id_fk"
...
ALTER TABLE "yield_positions" ADD CONSTRAINT "yield_positions_user_id_users_id_fk"
  FOREIGN KEY ("user_id") REFERENCES "users"("id");
